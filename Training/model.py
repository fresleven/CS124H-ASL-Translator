...
class Net(nn.Module):
    def __init__(self, num_words=340, frames=60,size=256):
        ...
        self.relu = nn.ReLU()
        self.preds1 = nn.Linear(int(256*(frames/4)*(size/128)*(size/128)), num_words)
    ...
    def forward(self, x):
        ...
        x = tmp + x 
        x = self.pool2(x) # C T H W = 128 self.frames self.size/4 self.size/4
        tmp = x
...
if __name__ == "__main__":
    model = Net().cuda()
    x = torch.rand(1, 3, 60, 256, 256).cuda()
    with torch.no_grad():
        y = model(x)
